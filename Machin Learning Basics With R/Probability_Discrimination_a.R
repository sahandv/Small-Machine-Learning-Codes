...
G2_x = function(x)
{
  x= log(1/((4*pi)^(1/2)),base = exp(1))-(((x-1)^2)/4)+log(1/2,base = exp(1))
}
...
lines(C1_dens$x,sapply(C1_dens$x,function(x) log(1/((4*pi)^(1/2)),base = exp(1))-(((x-1)^2)/4)+log(1/2,base = exp(1))) , col = "blue")
grid(25, col = "lightgray", lty = "dotted", lwd = 1)
title("G1 (red) and G2 (blue) using samples of classes")
#OR
plot(C2_dens$x,sapply(C2_dens$x,function(x) log(1/((2*pi)^(1/2)),base = exp(1))-((x^2)/2)+log(1/2,base = exp(1))), type = "l" , col = "red")
lines(C2_dens$x,sapply(C2_dens$x,function(x) log(1/((4*pi)^(1/2)),base = exp(1))-(((x-1)^2)/4)+log(1/2,base = exp(1))) , col = "blue")
title("G1 (red) and G2 (blue) using samples of classes")
